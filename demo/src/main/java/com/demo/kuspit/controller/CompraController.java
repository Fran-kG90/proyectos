...
import com.demo.kuspit.service.CompraService;
...
public class CompraController {
	 @Autowired
	    private CompraService offersService;
	    ...
	    public String setOffer(@ModelAttribute Compra offer, Principal principal) {
	    ...
	    public String updateList(Model model, HttpSession httpSession, Pageable pageable, Principal principal) {
	        ...
	        User user = usersService.getUserByEmail(email);
	        Page<Compra> offers = offersService.getOffersAvailableForUser(pageable, user);
	        model.addAttribute("offerList", offers.getContent());
	    ...
	    public String updateBoughtList(Model model, HttpSession httpSession, Pageable pageable, Principal principal) {
	        String email = principal.getName();
	        User user = usersService.getUserByEmail(email);
	        Page<Compra> offers = offersService.getBoughtOffers(pageable, user);
	        model.addAttribute("boughtList", offers.getContent());
	        httpSession.setAttribute("money", usersService.getActualUserMoney());
	    ...
	    public String setEdit(Model model, Principal principal, @PathVariable Long id, @ModelAttribute Compra offer) {
	        User user = usersService.getUserByEmail(principal.getName());
	        Compra original = offersService.getOffer(id);
	    ...
	    public String setBuyTrue(Model model, HttpSession httpSession, Principal principal, @PathVariable Long id) {
	        User user = usersService.getUserByEmail(principal.getName());
	        Compra offer = offersService.getOffer(id);
